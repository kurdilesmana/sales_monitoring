...
define("E_VAL_SHOULD_SEL_CHECK_FAILED","Wrong option selected for %s");
...
class FormValidator 
{
	...
	function validate_wen($input_value, &$default_error_message,$variable_name)
	{
	  $bret = true;
	  	if(is_array($input_value)){
			foreach($input_value as $listdata){
				if($listdata <= 0 || preg_match('[^0-9]',$listdata))
				{
					$bret=false;
			...
			if($input_value <= 0 || preg_match('[^0-9]',$input_value))
			{
				$bret=false;
				$default_error_message = sprintf(E_VAL_NUM_CHECK_FAILED,$variable_name);
			}
	...
	function ValidateCommand($command,$command_value,$input_value,&$default_error_message,$variable_name,$formvariables)
	{
		...
		switch($command)
		{
			...
			case 'numberwendy':
						{
							$bret = $this->validate_wen($input_value, $default_error_message,$variable_name);
							break;		
						}
